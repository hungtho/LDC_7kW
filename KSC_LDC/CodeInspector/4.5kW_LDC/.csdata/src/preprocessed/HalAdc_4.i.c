#line 1 "D:\\07_Working\\KSC_LDC\\source\\HAL\\HalAdc.c"
...
#line 36 "D:\\07_Working\\KSC_LDC\\source\\HAL\\HalAdc.c"
...
#line 38 "D:\\07_Working\\KSC_LDC\\source\\HAL\\LdcConfig.h"
...
#line 41 "D:\\07_Working\\KSC_LDC\\source\\HAL\\HalPIT.h"
...
#line 37 "D:\\07_Working\\KSC_LDC\\source\\HAL\\HalAdc.c"
...
static uint16_t HalMovingAvg ( const uint16_t ptrArrNumbers[] );
static uint16_t CpuInADC ( uint8_t u8_AdcCh );
static void HalAdcBuffer ( uint8_t chn, uint8_t buffer_Num, uint16_t buffer[] );
...
uint16_t Vout[ 19 ] = { 110,120,130,140,150,160,170,180,190,200,210,220,230,250,270,290,310,330,350 };
uint16_t AdcVout[ 19 ] = { 1185,1292,1401,1508,1616,1723,1832,1940,2045,2156,2264,2371,2484,2692,2907,3118,3386,3584,3768 };
uint16_t AdcIout[ 19 ] = { 415,576,737,901,1066,1230,1393,1557,1726,1887,2243,2386,2551,2717,2883,3050,3203,3353,3503 }; 
uint16_t Iout[ 19 ] = { 0,100,200,300,400,500,600,700,800,900,1100,1200,1300,1400,1500,1600,1700,1800,1900 };


uint16_t AverAdc_Iout = 0U;
uint16_t AverAdc_Temp = 0U;
uint16_t AverAdc_KeyIn = 0U;


volatile uint32_t AdcVout_Q15 = 0U;
volatile uint32_t AdcIout_Q15 = 0U;
...
static volatile struct SARADC_tag *stsp_SAR_ADC_12BIT_0 = &(*(volatile struct SARADC_tag *) 0xffe00000UL);
...
static void HalAdc0Init( void )
{
	SPCSetPeripheralClockMode( 127, (((uint8_t)(2)) << 0) | (((uint8_t)(2)) << 3) );

	stsp_SAR_ADC_12BIT_0->PDEDR.R = 0U;			 
	 
	stsp_SAR_ADC_12BIT_0->MCR.B.PWDN = 0U;
	stsp_SAR_ADC_12BIT_0->MCR.B.NTRGEN = 0U;  

	 
	stsp_SAR_ADC_12BIT_0->MCR.B.MODE = 0U;
	 
	stsp_SAR_ADC_12BIT_0->MCR.B.OWREN = 1U;
	 
	stsp_SAR_ADC_12BIT_0->MCR.B.FRZ = 1U;

	 
	stsp_SAR_ADC_12BIT_0->CTR[ 0 ].B.CRES = 0U;	 
	stsp_SAR_ADC_12BIT_0->CTR[ 0 ].B.PRECHG = 1U;	 
	stsp_SAR_ADC_12BIT_0->CTR[ 0 ].B.INPSAMP = 5U;  
	stsp_SAR_ADC_12BIT_0->ICDR[ 0 ].R = 0x0U;
...
void Hal_Adc_Cal ( void )
{
	static const uint16_t AdcBplus[ 19 ] = { 1176,1283,1387,1493,1600,1714,1823,1927,2024,2144,2255,2354,2462,2674,2886,3100,3316,3504,3690 };
	static const uint16_t VBlus[ 19 ] = { 110,120,130,140,150,160,170,180,190,200,210,220,230,250,270,290,310,330,350 };

	static const uint16_t AdcVin[ 19 ] = { 45,160,318,476,635,794,952,1112,1271,1430,1589,1748,1907,2067,2224,2383,2435,2541,2648 };
	static const uint16_t Vin[ 19 ] = { 0,300,600,900,1200,1500,1800,2100,2400,2700,3000,3300,3600,3900,4200,4500,4600,4800,5000 };

	static const uint16_t AdcTemp[ 19 ] = { 360,446,503,555,614,695,771,868,967,1082,1341,1644,1984,2346,2708,3190,3574,3879,3965 };
	static const uint16_t Temp[ 19 ] = { 160,150,145,140,135,130,125,120,115,110,100,90,80,70,60,45,30,10,0 };

#line 174 "D:\\07_Working\\KSC_LDC\\source\\HAL\\HalAdc.c"

	static uint16_t Hist_KeyIn[ ( 10U ) ];
	static uint16_t Hist_14VVCC[ ( 10U ) ];
	static uint16_t Hist_Vout[ ( 10U ) ];
	static uint16_t Hist_Iout[ ( 10U ) ];
	static uint16_t Hist_Vin[ ( 10U ) ];
	static uint16_t Hist_Temp[ ( 10U ) ];

	static uint16_t AverAdc_14VVCC = 0U;
	static uint16_t AverAdc_Vout = 0U;
	static uint16_t AverAdc_Vin = 0U;

	HalAdcBuffer ( 2U, 0U, Hist_KeyIn );
	HalAdcBuffer ( 3U, 1U, Hist_14VVCC );
	HalAdcBuffer ( 5U, 2U, Hist_Vout );
	HalAdcBuffer ( 6U, 3U, Hist_Iout );
	HalAdcBuffer ( 7U, 4U, Hist_Vin );
	HalAdcBuffer ( 9U, 5U, Hist_Temp );

	AverAdc_KeyIn = HalMovingAvg( Hist_KeyIn );
	AverAdc_14VVCC = HalMovingAvg( Hist_14VVCC );
	AverAdc_Vout = HalMovingAvg( Hist_Vout );
	AverAdc_Iout = HalMovingAvg( Hist_Iout );
	AverAdc_Vin = HalMovingAvg( Hist_Vin );
	AverAdc_Temp = HalMovingAvg( Hist_Temp );

	realBplus = UTILS_ADC2REAL( AverAdc_14VVCC, AdcBplus, VBlus );
	realVout = UTILS_ADC2REAL( AverAdc_Vout, AdcVout, Vout );
	realIout = UTILS_ADC2REAL( AverAdc_Iout, AdcIout, Iout );
	realVin = UTILS_ADC2REAL( AverAdc_Vin, AdcVin, Vin );
	realTemp = ( uint16_t )UTILS_ADC2REAL_TEMP( AverAdc_Temp, AdcTemp, Temp );


	if( AverAdc_Iout >= ( ( 409U ) + 1U ) )
	{
		AdcIout_Q15 = ( AverAdc_Iout - ( 409U ) ) << 5U ;				 
	}
	else
	{
		AdcIout_Q15 = 0U;
...
static uint16_t HalMovingAvg ( const uint16_t ptrArrNumbers[] )
{
  ...
  Sum = Sum - ( ( uint32_t )u16_Max + ( uint32_t )u16_Min );

  AvgRet = ( uint16_t )( Sum / ( ( 10U ) - 2U ) );
...
static uint16_t CpuInADC ( uint8_t u8_AdcCh )
{
	...
	u32_AdcChReg = 0x00000001U << u8_AdcCh;

	stsp_SAR_ADC_12BIT_0->ICNCMR[ 0 ].R = u32_AdcChReg;
	stsp_SAR_ADC_12BIT_0->MCR.B.NSTART = 0x1U;

	while( stsp_SAR_ADC_12BIT_0->ICDR[ u8_AdcCh ].B.VALID == 1U ){};

	return ( ( uint16_t )( stsp_SAR_ADC_12BIT_0->ICDR[ u8_AdcCh ].B.CDATA & 0x0FFFU ) );
}
...
static void HalAdcBuffer ( uint8_t chn, uint8_t buffer_Num, uint16_t buffer[] )
{
	static uint16_t Index[ ( 6U ) ];

	if( buffer_Num < 6U )
	{
		buffer[ Index[ buffer_Num ] ] = CpuInADC ( chn );	 ;

		Index[ buffer_Num ]++;
		if( Index[ buffer_Num ] >= ( 10U ) ){ Index[ buffer_Num ] = 0U; }
	}
}
